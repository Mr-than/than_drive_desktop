...
import com.sun.jna.platform.win32.WinUser.LWA_ALPHA
...
private interface User32Ex : User32 {
    fun SetWindowLong(hWnd: WinDef.HWND, nIndex: Int, wndProc: WinUser.WindowProc): BaseTSD.LONG_PTR
    fun SetWindowLongPtr(hWnd: WinDef.HWND, nIndex: Int, wndProc: WinUser.WindowProc): BaseTSD.LONG_PTR
    fun CallWindowProc(proc: BaseTSD.LONG_PTR, hWnd: WinDef.HWND, uMsg: Int, uParam: WinDef.WPARAM, lParam: WinDef.LPARAM): WinDef.LRESULT
}
...
class CustomWindowProcedure(private val windowHandle: WinDef.HWND) : WinUser.WindowProc {
    ...
    private var defaultWndProc = if (is64Bit()) {
        USER32EX?.SetWindowLongPtr(windowHandle, GWLP_WNDPROC, this) ?: BaseTSD.LONG_PTR(-1)
    } else {
        USER32EX?.SetWindowLong(windowHandle, GWLP_WNDPROC, this) ?: BaseTSD.LONG_PTR(-1)
    }
    ...
    override fun callback(hWnd: WinDef.HWND, uMsg: Int, wParam: WinDef.WPARAM, lParam: WinDef.LPARAM): WinDef.LRESULT {
        return when (uMsg) {
            ...
            WM_NCHITTEST -> { USER32EX?.CallWindowProc(defaultWndProc, hWnd, uMsg, wParam, lParam) ?: WinDef.LRESULT(
                0
            )
            }
            WM_DESTROY -> { USER32EX?.CallWindowProc(defaultWndProc, hWnd, uMsg, wParam, lParam) ?: WinDef.LRESULT(
                0
            )
            }
            else -> { USER32EX?.CallWindowProc(defaultWndProc, hWnd, uMsg, wParam, lParam) ?: WinDef.LRESULT(
                0
            )
            }
        }
